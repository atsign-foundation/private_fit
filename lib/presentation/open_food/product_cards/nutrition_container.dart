...
class NutritionContainer {
  NutritionContainer({
    ...
    final json = product.nutriments?.toJson();
    if (json != null) {
      _loadUnits(json);
      _loadValues(json);
    }
  ...
  bool _isNotRelevant(final OrderedNutrient orderedNutrient) {
    final nutrientId = orderedNutrient.id;
    final value100g = getValue(getValueKey(nutrientId, false));
    final valueServing = getValue(getValueKey(nutrientId, true));
  ...
  Nutriments _getNutriments() {
    ...
    for (final orderedNutrient in getDisplayableNutrients()) {
      final nutrientId = orderedNutrient.id;
      final key100g = getValueKey(nutrientId, false);
      final keyServing = getValueKey(nutrientId, true);
      final value100g = getValue(key100g);
      final valueServing = getValue(keyServing);
      if (value100g == null && valueServing == null) {
        continue;
      }
      final unit = getUnit(nutrientId);
      if (value100g != null) {
  ...
  void setNextWeightUnit(final OrderedNutrient orderedNutrient) {
    final unit = getUnit(orderedNutrient.id);
  ...
  void _loadUnits(final Map<String, dynamic> json) {
    for (final orderedNutrient in _nutrients) {
      final nutrientId = orderedNutrient.id;
      final unitKey = _getNutrimentsUnitKey(nutrientId);
      final dynamic value = json[unitKey];
      if (value == null || value is! String) {
        continue;
      }
      final unit = UnitHelper.stringToUnit(value);
      if (unit != null) {
  ...
  void _loadValues(final Map<String, dynamic> json) {
    ...
    for (final orderedNutrient in _nutrients) {
      ...
      final unit = getUnit(nutrientId);
      for (var i = 0; i < 2; i++) {
        final perServing = i == 0;
        final valueKey = getValueKey(nutrientId, perServing);
        final value = _convertWeightFromG(
