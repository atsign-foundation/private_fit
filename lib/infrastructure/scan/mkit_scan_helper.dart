...
import 'package:private_fit/infrastructure/scan/abstract_camera_image_getter.dart';
// import 'package:private_fit/infrastructure/scan/camera_image_cropper.dart';
import 'package:private_fit/infrastructure/scan/camera_image_full_getter.dart';
...
class _MLKitScanDecoderMainIsolate {
  ...
  /// is [_isIsolateInitialized=true]. This variable temporary contains the
  /// image waiting to be decoded.
...
class _MLKitScanDecoderIsolate {
  ...
  static Future<void> onNewBarcode(
    ...
    final image = CameraImage.fromPlatformData(message);
    final cropImage = _cropImage(image);
    final imageHeight =
        cropImage.inputImageData?.size.longestSide ?? double.infinity;

    final barcodes = await _barcodeScanner.processImage(cropImage);
