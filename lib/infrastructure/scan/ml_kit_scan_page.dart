...
import 'package:private_fit/infrastructure/scan/camera_controller.dart';
import 'package:private_fit/infrastructure/scan/camera_helper.dart';
import 'package:private_fit/infrastructure/scan/collections_helper.dart';
import 'package:private_fit/infrastructure/scan/continuous_scan_model.dart';
import 'package:private_fit/infrastructure/scan/lifecycle_aware_widget.dart';
import 'package:private_fit/infrastructure/scan/lifecycle_manager.dart';
import 'package:private_fit/infrastructure/scan/mkit_scan_helper.dart';
import 'package:private_fit/infrastructure/scan/scan_visor.dart';
import 'package:private_fit/infrastructure/scan/screen_visibility.dart';
...
class MLKitScannerPageState extends LifecycleAwareState<MLKitScannerPage> {
  ...
  void _restartCameraIfNecessary() {
    if (mounted && ScreenVisibilityDetector.visible(context)) {
      ...
      SchedulerBinding.instance.addPostFrameCallback((_) {
        final diff = DateTime.now().difference(referentialTime).inMilliseconds;
  ...
  _FocusPoint get _focusPoint {
    ...
    if (mounted) {
      final key = Provider.of<GlobalKey<ScannerVisorWidgetState>>(
        context,
        listen: false,
      );

      final visorOffset = (key.currentContext?.findRenderObject() as RenderBox?)
